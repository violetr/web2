...
UseR!2019 Datathon submission - 3rd place
...
In this document I present my analysis of the "Health, Nutrition and Population Statistics" dataset hosted by the World Bank Group. It was compiled from an RMarkdown file `datathon2019.Rmd`. A Shiny App showing the discussed topic and other visualizations can be accessed [here](https://violetr.shinyapps.io/hiv_hnp/). It's a complementary material but it doesn't replace this report. All the code that I used to build both the document and the application is available in this repository: github.com/violetr/health. 
...
#saveRDS(countries_simple, file = here::here("data_health", "countries_simple.rds"))
...
data_to_embed <- function(indicators_hnp_keep, dataset_hnp_health, an_year, custom.config) {
  ...
  dataset_comparable_health <- dataset_hnp_health %>%
    ...
    select_if(function(x) (sum(is.na(x)) <= (0.47 * length(x))) || is.character(x))
...
dataset_comparable_health <- dataset_hnp_health %>%
  filter(year == an_year) %>%
  select(country_name, country_code, region, one_of(comparable_health_indicators)) %>%
  select_if(function(x) (sum(is.na(x)) <= (0.46 * length(x))) || is.character(x))
